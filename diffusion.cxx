...
{
    u1[0]=D*dt/pow(dx,2)*(u0[1]-2*u0[0]+u0[N-1])+u0[0]; //periodic boundaries
    
    for(int i=1; i<N-1; i++)
        u1[i]=D*dt/pow(dx,2)*(u0[i+1]-2*u0[i]+u0[i-1])+u0[i];
        
    u1[N-1]=D*dt/pow(dx,2)*(u0[0]-2*u0[N-1]+u0[N-2])+u0[N-1]; //periodic boundaries
